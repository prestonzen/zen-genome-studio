import { Database, Dna, ExternalLink, Eye, EyeOff, GitCompare, Globe2, RefreshCw, ShieldCheck, Waypoints } from 'lucide-react'
...
export function AncestryView({ report, loading, recordSafe, onRefresh, onToggleSafe }: AncestryViewProps) {
  ...
  const rows = mergeRegions(self, parent)
  ...
  return (
    <main className="workspace ancestry-workspace">
      <section className="ancestry-intro">
        ...
        <button type="button" onClick={onToggleSafe}>{recordSafe ? <Eye size={17} /> : <EyeOff size={17} />}{recordSafe ? 'Reveal privately' : 'Hide for recording'}</button>
      </section>
      ...
      <div className="ancestry-layout">
        <section className="ancestry-main">
          ...
          {!ready ? (
            ...
            <div className="ancestry-lock"><ShieldCheck size={28} /><div><strong>Ancestry details hidden</strong><p>Record-safe mode conceals personal regions, percentages, and family comparisons.</p></div></div>
          ) : (
            <div className="ancestry-results">
              <div className="ancestry-profile-strips">
                ...
                {parent && <div><span><strong>{parent.label}</strong><small>Family comparison only</small></span><RegionStrip profile={parent} /></div>}
              </div>
              <div className="ancestry-table" role="table" aria-label="Imported ancestry region comparison">
                <div className="ancestry-table-head" role="row"><span role="columnheader">Region</span><span role="columnheader">{self?.label}</span><span role="columnheader">{parent?.label ?? 'Parent'}</span></div>
                {rows.map((row) => (
                  <div className="ancestry-row" role="row" key={row.id}>
                    ...
                    <PercentCell value={row.parentPercent} color={row.color} label={parent?.label ?? 'Parent'} />
                  </div>
                ))}
              </div>
              <div className="ancestry-family-note"><GitCompare size={20} /><p><strong>Do not average parent and child percentages.</strong> You inherit a random half of each parent's DNA, and each company update can move boundaries or relabel reference groups.</p></div>
            </div>
          )}
        </section>
